					...
					<el-row>
						<el-col :span="11">
							<div class="description">
								<el-tag
										v-if="res.length !== 0 && res[i - 1].id !== 0 && res[i - 1].id !== 4 && res[i - 1].id !== 5 && res[i - 1].id < 12 && res[i - 1].level === 4"
										type="info" size="small" style="float:left;">{{ tags[res[i - 1].id] }}
								</el-tag>
								<el-tag
										v-if="res.length !== 0 && res[i - 1].id !== 0 && res[i - 1].id !== 4 && res[i - 1].id !== 5 && res[i - 1].id < 12 && res[i - 1].level === 5"
										type="danger" size="small" style="float:left;">{{ tags[res[i - 1].id] }}
								</el-tag>
								<el-tag v-if="res.length !== 0 && res[i - 1].id !== 0 && res[i - 1].id === 12"
										type="warning"
						...
						<el-col :span="11">
							<div class="description">
								<el-tag
										v-if="res1.length !== 0 && res1[i - 1].id !== 0 && res1[i - 1].id !== 4 && res1[i - 1].id !== 5 && res1[i - 1].id < 12 && res1[i - 1].level === 4"
										type="info" size="small" style="float:left;">{{ tags[res1[i - 1].id] }}
								</el-tag>
								<el-tag
										v-if="res1.length !== 0 && res1[i - 1].id !== 0 && res1[i - 1].id !== 4 && res1[i - 1].id !== 5 && res1[i - 1].id < 12 && res1[i - 1].level === 5"
										type="danger" size="small" style="float:left;">{{ tags[res1[i - 1].id] }}
								</el-tag>
								<el-tag v-if="res1.length !== 0 && res1[i - 1].id !== 0 && res1[i - 1].id === 12"
										type="warning"
										size="small" style="float:left;">忘川风华
								</el-tag>
...
export default {
    ...
    data() {
        return {
            ...
            ly: 0,
            hc: 0,
            bd: Math.floor(Math.random() * 50) + 200,
            res: [],
            res1: [],
            levelProbability: [
                [10, 10, 10, 10, 25, 35],
            ...
            probability: [
                [0, 0, 0, 0, 15, 15, 10, 10, 10, 0, 0, 10, 2, 0, 0, 15, 15],
                [0, 0, 0, 0, 15, 15, 10, 10, 10, 0, 0, 10, 2, 0, 0, 15, 15],
                [15, 10, 0, 10, 15, 0, 10, 0, 0, 10, 0, 0, 2, 15, 15, 0, 0],
                [15, 10, 10, 0, 0, 15, 0, 10, 0, 0, 10, 0, 2, 15, 15, 0, 0],
                [10, 10, 10, 10, 0, 0, 0, 0, 0, 10, 10, 0, 2, 10, 10, 10, 10]
            ],
            w1: [1, 10, 0, 0],
            w2: [1, 15, 4, 20],
        ...
        getList() {
            ...
            for (let i = 0; i < 4; i++) {
                if (!this.locked[i]) {
                    ...
                    if (id === 12) {
                        level = 5;
                        this.bd = Math.floor(Math.random() * 50) + 200;
                    } else {
                        level = this.getLevel();
                        --this.bd;
                    }
